...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ FONCTION 1/3 : showSection() - VERSION CORRIGÉE
    async function showSection(targetId, options = {}) {
        ...
        // ✅ NOUVEAU : Émettre l'événement de changement d'interface
        const currentInterface = document.querySelector('.content-section.active')?.id;
        if (currentInterface && currentInterface !== targetId) {
            document.dispatchEvent(new CustomEvent('kioskInterfaceChange', {
                detail: { from: currentInterface, to: targetId }
            }));
        }
        ...
        // Initialiser canvas + bouton plein écran pour les interfaces kiosque
        if (targetId === 'kiosk-interface-1') {
            setTimeout(() => {
                // Créer canvas
                if (!window.kioskCanvasManager.get('kiosk-canvas-1')) {
                    window.kioskCanvasManager.create('kiosk-canvas-1', 'kiosk-interface-1');
                } else {
                    // ✅ CORRECTION : Régénérer si existe déjà
                    window.kioskCanvasManager.get('kiosk-canvas-1').forceRegenerate();
                }
                
                // Créer bouton plein écran
                const container = document.querySelector('#kiosk-interface-1 .kiosk-preview-container');
                if (container && !container.querySelector('.kiosk-fullscreen-toggle')) {
                    const toggleBtn = window.kioskFullscreenManager.createToggleButton('kiosk-interface-1', 'kiosk-canvas-1');
        ...
        if (targetId === 'kiosk-view') {
            setTimeout(() => {
                // Créer canvas
                if (!window.kioskCanvasManager.get('kiosk-canvas-2')) {
                    window.kioskCanvasManager.create('kiosk-canvas-2', 'kiosk-view');
                } else {
                    // ✅ CORRECTION : Régénérer si existe déjà
                    window.kioskCanvasManager.get('kiosk-canvas-2').forceRegenerate();
                }
                
                // Créer bouton plein écran
                const container = document.querySelector('#kiosk-view .kiosk-preview-container');
                if (container && !container.querySelector('.kiosk-fullscreen-toggle')) {
                    const toggleBtn = window.kioskFullscreenManager.createToggleButton('kiosk-view', 'kiosk-canvas-2');
                    container.appendChild(toggleBtn);
                }
            }, 100);
        }
        ...
        if (targetId === 'kiosk-view') {
            ...
            // ✅ CORRECTION : Gérer sidebar ET état plein écran
            const wasFullscreen = window.kioskFullscreenManager && window.kioskFullscreenManager.isInFullscreen();
            
            if (mode === 'admin' || wasFullscreen) {
                // Mode admin OU plein écran : Cacher sidebar
                ...
                // Mode client normal : Garder sidebar
        ...
        if (targetId === 'kiosk-interface-1') {
            ...
            // ✅ CORRECTION : Gérer sidebar selon l'état plein écran
            const wasFullscreen = window.kioskFullscreenManager && window.kioskFullscreenManager.isInFullscreen();
            
            if (wasFullscreen) {
                // En plein écran : Cacher sidebar
                if (sidebar) sidebar.style.display = 'none';
                if (mainContent) {
                    mainContent.style.marginLeft = '0';
                    mainContent.style.padding = '0';
    ...
    // ✅ FONCTION 3/3 : Bouton retour gérant - VERSION CORRIGÉE
    if (toggleBackBtn) {
        ...
        newToggleBackBtn.addEventListener('click', async (e) => {
            ...
            // ✅ CORRECTION : Réafficher la sidebar avec menu-item active
            const sidebar = document.getElementById('sidebar');
            const mainContent = document.getElementById('mainContent');
            ...
            // ✅ CORRECTION : Réactiver le menu-item
            const menuItems = document.querySelectorAll('.menu-item');
            menuItems.forEach(item => item.classList.remove('active'));
            
            const gestionMenuItem = document.querySelector('.menu-item[data-target="gestion"]');
            if (gestionMenuItem) {
                gestionMenuItem.classList.add('active');
                console.log('✅ [Interface-App] Menu-item "Gestion" réactivé');
            }
    ...
    // ✅ FONCTION 2/3 : Bouton retour client - VERSION CORRIGÉE
    const kioskBackBtn = document.getElementById('kiosk-back-btn');
    if (kioskBackBtn) {
        // Supprimer tous les listeners existants
        const newKioskBackBtn = kioskBackBtn.cloneNode(true);
        kioskBackBtn.parentNode.replaceChild(newKioskBackBtn, kioskBackBtn);
        
        newKioskBackBtn.addEventListener('click', async (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('🔙 [Interface-App] Retour Interface 1 (client)');
            
            // ✅ CORRECTION : Forcer la régénération du canvas Interface 1
            setTimeout(() => {
                const canvas1 = window.kioskCanvasManager.get('kiosk-canvas-1');
                if (canvas1) {
                    canvas1.forceRegenerate();
                    console.log('✅ [Interface-App] Canvas Interface 1 régénéré');
                }
            }, 150);
            
            // Retour à Interface 1
